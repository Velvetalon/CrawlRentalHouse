...
#     def __init__(self):
#         self.proxy = ProxyAPI.ProxyAPI()
#     def process_exception(self,request,exception,spider):
#         #超时重试
#         if isinstance(exception, (Terror.TimeoutError,Terror.ConnectionRefusedError)):
#             #访问失败时移除不可用代理。
#             if request.meta.get('proxy',None) :
#                 self.proxy.delete_proxy(request.meta['proxy'],0)
#                 #print("访问超时，已移除代理：",request.meta['proxy'])
...
#             if request.meta.get('proxy', None):
#                 self.proxy.delete_proxy(request.meta['proxy'],0)
#                 #print("访问失败，已移除代理：",request.meta['proxy'])
#                 request.meta["proxy"] = self.proxy.get_proxy()
#             return request
#         return response
